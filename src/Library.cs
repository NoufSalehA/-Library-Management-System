...
{//all the work
    ...
    private INotificationService _notificationService;//receive what inside constructor down
    ...
    public void AddBook(Book book)//method for adding books
    {
        ...
        _notificationService.SendNotificationOnSuccess($"*{book.Name}* Has been Added");
    }
    public void DeleteBook(Guid id)//method for deleting books by its id
    {
        ...
        if (bookToDelete != null)
        {
            ...
            _notificationService.SendNotificationOnSuccess($"*{bookToDelete.Name} *--- Deleted ---");
        }
        else
        {
            _notificationService.SendNotificationOnFailure($"--- Incorrect ID---");
        }
    }
    ...
    {
        _users.Add(user);
        _notificationService.SendNotificationOnSuccess($"*{user.Name}* Has been Added");
    }
    public void DeleteUser(Guid id)
    {
        ...
        if (userToDelete != null)
        {
            ...
            _notificationService.SendNotificationOnFailure($"User{userToDelete.Name} is removed from list");
        }
        else
        {
